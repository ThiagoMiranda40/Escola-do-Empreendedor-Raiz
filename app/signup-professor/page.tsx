...
export default function SignupProfessorPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Create auth user
      // O perfil será criado automaticamente pelo Trigger que configuramos no Banco de Dados
      const { data: authData, error: authError } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            full_name: fullName,
            role: 'TEACHER', // Este dado será usado pelo Trigger para popular o perfil
          },
      ...
      // Se o email não precisar de confirmação, redireciona. 
      // Se precisar, o Supabase retornará uma sessão nula aqui inicialmente.
      if (authData.session) {
